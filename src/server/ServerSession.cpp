...
void appendQueryCacheTelemetry(jsonrpc::Json& result,
    ...
    result["navigationOccurrenceScanned"] = stats.navigation_occurrence_scanned;
    result["navigationTargetLookupScanned"] = stats.navigation_target_lookup_scanned;
    ...
    result["queryCacheHoverEntries"] = stats.hover_entries;
    result["queryCacheDefinitionEntries"] = stats.definition_entries;
    result["queryCacheTypeDefinitionEntries"] = stats.type_definition_entries;
    result["queryCacheImplementationEntries"] = stats.implementation_entries;
    result["queryCachePrepareRenameEntries"] = stats.prepare_rename_entries;
    result["queryCacheDocumentHighlightEntries"] = stats.document_highlight_entries;
        ...
        result["gdsMetrics"] = jsonrpc::Json{
            ...
            {"parseMetrics",
             jsonrpc::Json{{"readMicros", info.gds_open_metrics.parse.read_micros},
                           ...
                           {"elementFinalizeBboxMicros",
                            info.gds_open_metrics.parse.element_finalize_bbox_micros},
                           {"elementFinalizeReferenceMicros",
                            info.gds_open_metrics.parse.element_finalize_reference_micros},
                           {"elementFinalizeIndexMicros",
                            info.gds_open_metrics.parse.element_finalize_index_micros},
                           {"elementFinalizeSampleMicros",
                            info.gds_open_metrics.parse.element_finalize_sample_micros},
                           {"diagnosticMicros", info.gds_open_metrics.parse.diagnostic_micros},
                           {"resolveMicros", info.gds_open_metrics.parse.resolve_micros},
                           {"resolveLookupMicros", info.gds_open_metrics.parse.resolve_lookup_micros},
                           {"resolveReferenceMicros",
                            info.gds_open_metrics.parse.resolve_reference_micros},
                           {"resolveTopCellMicros", info.gds_open_metrics.parse.resolve_top_cell_micros},
...
jsonrpc::Json ServerSession::handleSchematic(const jsonrpc::Json& params) {
    ...
    if (schematic.schematic_connection_fact_lookup_count > 0) {
        result["schematicConnectionFactLookups"] = schematic.schematic_connection_fact_lookup_count;
    }
    if (schematic.schematic_source_part_scan_count > 0) {
        result["schematicSourcePartScans"] = schematic.schematic_source_part_scan_count;
    }
...
jsonrpc::Json ServerSession::handleBackwardCone(const jsonrpc::Json& params) {
    ...
    if (trace.cone_assertion_sample_edge_count > 0) {
        result["coneAssertionSampleEdges"] = trace.cone_assertion_sample_edge_count;
    }
    if (trace.cone_assertion_clock_edge_count > 0) {
        result["coneAssertionClockEdges"] = trace.cone_assertion_clock_edge_count;
    }
    if (trace.cone_assertion_disable_edge_count > 0) {
        result["coneAssertionDisableEdges"] = trace.cone_assertion_disable_edge_count;
    }
    ...
    if (trace.cone_assertion_invocation_edge_count > 0) {
        result["coneAssertionInvocationEdges"] = trace.cone_assertion_invocation_edge_count;
    }
