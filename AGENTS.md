...
- current validation scale is 457+ focused unit `TEST_CASE`s and 409+ JSON semantic golden fixtures; the next maturity target is 480+ unit `TEST_CASE`s and 445+ golden fixtures focused on control/data cone facts, URI-local graph build views, visible graph-query affected rebuild, shared-LSP differential, and required synthetic perf coverage rather than debt-only fixtures
...
- design graph queries use build-time value facts: `DesignGraphIndexBuilder` is the sole graph binding and adjacency construction entry point; do not leave a legacy builder or duplicate helper path in `AstIndex.cpp`. `DesignGraphBindingIndex` owns exact URI/range-to-symbol, module-scope/name, port, parameter, generated-instance, and `GraphEndpointFact` bindings, including URI-local `SnapshotGraphConnectionBindingFact` source identity sets for named, positional, parameter-override, and complex connections. Endpoint kind, port direction, cone edge kind, source role, and slice kind are typed enums; raw `SnapshotAssignmentEdge` facts carry only build-time data/control dependency facts. `ConeAdjacencyIndex` owns typed assignment, `ControlDependency`, instance-port, and parameter-override edges with generated-instance identity in both directions. Build-time declaration recovery is limited to a deterministic URI-local AST declaration view; provider queries may not scan `symbols_by_id`, module entries, per-URI assignment maps, or reconstruct graph identity by name/range. Missing module endpoint bindings return `partial/messages`; they do not re-enable syntax, text, workspace, or port-name direction guesses such as `y/out/o/q`. `graphScannedGlobalSymbols` and `coneScannedGlobalEdges` must stay zero in focused, synthetic perf, and RTL E2E summaries; synthetic cross-module cone probes must also demonstrate a nonzero local adjacency scan and a successful warm query. `if` and `case` control sources are indexed; ternary control identity must remain an explicit unresolved/partial follow-up until it reaches the same query-root fact source.
