...
- current validation scale is 462+ focused unit `TEST_CASE`s and 409+ JSON semantic golden fixtures; the next maturity target is 480+ unit `TEST_CASE`s and 445+ golden fixtures focused on `InterfaceModportBindingIndex`, directional cone facts, URI-local graph build views, visible graph-query affected rebuild, shared-LSP differential, and required synthetic perf coverage rather than debt-only fixtures
