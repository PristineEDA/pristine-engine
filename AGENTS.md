...
- current validation scale is 233+ focused unit `TEST_CASE`s and 272+ JSON semantic golden fixtures; the next maturity target is true complex-SV provider behavior plus fixture-driven differential, cache, affected-rebuild, and perf coverage rather than debt-only fixtures
...
- expand AST-derived typeDefinition coverage for package-qualified typedefs, class/interface/modport type references, shadowed type names, and any remaining alias-chain edge cases; do not regress the positive typedef alias-chain fixture back to unresolved debt
