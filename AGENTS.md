...
- current validation scale is 389+ focused unit `TEST_CASE`s and 351+ JSON semantic golden fixtures; the next maturity target is 400+ unit `TEST_CASE`s and 375+ golden fixtures focused on AST-backed callable invocation and macro semantics, visible-query affected rebuild, fixture-driven differential, and required synthetic perf coverage rather than debt-only fixtures
...
- current semantic feature focus is AST-backed scope, callable invocation, and macro usage visibility: `AstIndex` owns lexical/enclosing scope candidates, URI-local `CallableInvocationFact` and `MacroInvocationFact` range indexes, explicit and wildcard imports, transitive package exports with cycle guards, typed receiver members, include-closure macro visibility, define/redefine/undef lifetimes, and bounded nested macro expansion. `CompletionProvider` owns completion context, filtering, deterministic precedence, dedupe, snippets, resolve metadata, and lower-bound workspace prefix selection; `SignatureInlayProvider` consumes only indexed callable/macro argument ranges; macro hover, definition, signature, inlay, and expand code actions consume the same invocation identity and expansion facts. Provider queries must not scan the full symbol table, all workspace callables or macros, use stable-id substring or fuzzy membership tests, locate call parentheses or macro definitions with text search, or fall back to `CompilationService` semantic answers. An unresolved package, receiver, callable, macro, or import returns no speculative global candidates and an explicit message
- callable and macro affected rebuild is explicit: `AffectedDependencyGraph` classifies `SemanticExport`, `CallableType`, and `MacroInclude` edges in addition to include/import/module/config edges. Package export, callable declaration, macro include, document, watched-file, and config changes must invalidate completion, signature, inlay, hover, definition, and code-action visible-query results; tests must assert changed external results, not only generation counters
- signature/inlay/perf telemetry records URI-local invocation and visible macro scan counts, and `scannedGlobalSymbols` must remain zero. The required 100/1000/5000-file synthetic perf gate exercises cold/warm completion, resolve, signature, inlay, macro definition/expand, cache stats, and candidate scans without machine-specific time thresholds
