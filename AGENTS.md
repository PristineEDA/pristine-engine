...
- Do not add `metadata_by_uri`, `assignments_by_uri`, or `identifiers_by_uri` syntax-derived maps to visible provider contexts. If lexical identifier data is temporarily needed for AST range enrichment inside indexing, keep it internal to `SnapshotBuilder` / `AstIndex` and do not expose it as a semantic answer source.
