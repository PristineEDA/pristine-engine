...
- Staged GDS open changed the large-file UX metric: TT tinyQV `systemverilog/layout/open` with `openMode=auto` now returns a deferred session and pipe endpoint in about 33ms locally, while `StatusRequest/PLST` reaches ready after about 11.77s with parse about 11.01s. TT e2e must report both `lsp_open_wall_micros` and `ready_wall_micros`; treat the former as UI responsiveness and the latter as parser/backend readiness. The same run measured overview max about 274ms, cold wide LOD1 max about 665ms, post-warm wide max about 129us, tile query p95 about 209ms, hit query p95 about 230us, and search query p95 about 123ms, with one pending pipe error observed before ready. Future staged-open work should add true parser progress/cancellation hooks before claiming mid-parse cancellation; for now close is safe and joins the background parse, but the parser main loop is still a single-threaded GDS record state machine.
