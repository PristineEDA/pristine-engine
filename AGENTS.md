...
- current validation scale is 480+ focused unit `TEST_CASE`s and 445+ JSON semantic golden fixtures; the next maturity target is 500+ unit `TEST_CASE`s and 470+ golden fixtures focused on typed control/data/slice cone facts, interface/modport graph bindings, visible graph-query affected rebuild, shared-LSP differential, and required synthetic perf coverage rather than debt-only fixtures
