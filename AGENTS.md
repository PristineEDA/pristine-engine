...
- current validation scale is 557+ focused unit `TEST_CASE`s and 522+ JSON semantic golden fixtures; the next maturity target is 580+ unit `TEST_CASE`s and 550+ golden fixtures focused on typed schematic cell-pin projection, identity-based net grouping, module/interface/parameter affected rebuild, shared-LSP differential, and required synthetic perf coverage rather than debt-only fixtures
