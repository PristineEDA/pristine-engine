...
- current validation scale is 589+ focused unit `TEST_CASE`s and 533+ JSON semantic golden fixtures; the next maturity target is 610+ unit `TEST_CASE`s and 560+ golden fixtures focused on assertion context/invocation facts, typed schematic cell-pin projection, identity-based net grouping, module/interface/parameter affected rebuild, shared-LSP differential, and required synthetic perf coverage rather than debt-only fixtures
