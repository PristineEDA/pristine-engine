...
- Discovery closure metrics are perf/debug value-type summaries only. They may report candidate/selected/missing documents and cache behavior, but they must not be serialized as mandatory LSP semantics or used to override `AstIndex` / design graph answers.
