...
- current validation scale is 439+ focused unit `TEST_CASE`s and 405+ JSON semantic golden fixtures; the next maturity target is 465+ unit `TEST_CASE`s and 435+ golden fixtures focused on `DesignGraphBindingIndex`, `ConeAdjacencyIndex`, visible graph-query affected rebuild, shared-LSP differential, and required synthetic perf coverage rather than debt-only fixtures
...
- design graph queries use build-time value facts: `DesignGraphBindingIndex` owns exact URI/range-to-symbol, module-scope/name, port, parameter, and generated-instance bindings; `ConeAdjacencyIndex` owns stable driver/load assignment and instance-port edges in both directions. `DesignGraphProvider` may directly look up node metadata by stable ID, but must not scan `symbols_by_id`, module entries, per-URI assignment maps, or reconstruct graph identity by name/range at query time. Missing bindings return `unresolved/messages`; they do not re-enable syntax, text, or workspace fallbacks. `graphScannedGlobalSymbols` and `coneScannedGlobalEdges` must stay zero in focused, synthetic perf, and RTL E2E summaries.
