...
- completion, completion resolve, signature help, diagnostics, inlay hints, semantic tokens, selection ranges, and workspace symbols use `SemanticEngine` value-type APIs; completion context detection and completion item/doc/snippet helpers are owned by `CompletionProvider`, and remaining context gaps belong in analysis instead of `ServerSession`
...
- split-out internal analysis providers for the large engine implementation: `src/analysis/semantic/SnapshotBuilder.*`, `src/analysis/semantic/AstIndex.*`, `src/analysis/semantic/NavigationProvider.*`, `src/analysis/semantic/CompletionProvider.*` for completion context detection, item construction, docs, snippets, filtering, and completion-specific helpers, `src/analysis/semantic/SignatureInlayProvider.*`, `src/analysis/semantic/DesignGraphProvider.*`, `src/analysis/semantic/CodeActionProvider.*`, `src/analysis/semantic/DiagnosticProvider.*`, and `src/analysis/semantic/QueryCache.*`
...
- Completion item `data` must be generated by the analysis layer and resolved through `SemanticEngine::resolveCompletion`; do not infer completion docs, snippets, ports, or macro bodies in `ServerSession`.
- Completion context detection for package `::`, hierarchical `.`, macro, and module-instantiation positions plus completion item/doc/snippet construction belongs in `CompletionProvider`; do not duplicate that logic in `SemanticEngine` or `ServerSession`.
