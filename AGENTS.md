...
- completion, completion resolve, signature help, diagnostics, inlay hints, semantic tokens, selection ranges, and workspace symbols use `SemanticEngine` value-type APIs; completion context detection, item assembly, resolve docs/snippets, and completion-specific metadata are owned by `CompletionProvider`; signature help and inlay hint assembly are owned by `SignatureInlayProvider`; remaining context gaps belong in analysis instead of `ServerSession`
...
- workspace symbol generation is owned by `AstIndex` behind `SemanticEngine` value-type APIs; repeated visible query results are cached by the generation-keyed query cache; do not add a second analysis fallback index
...
- split-out internal analysis providers for the large engine implementation: `src/analysis/semantic/SnapshotBuilder.*`, `src/analysis/semantic/AstIndex.*` for workspace symbol query assembly and AST index migration, `src/analysis/semantic/NavigationProvider.*`, `src/analysis/semantic/CompletionProvider.*` for completion context detection, item construction, resolve docs/snippets/data, filtering, and completion-specific query helpers, `src/analysis/semantic/SignatureInlayProvider.*` for signature help and inlay hint query assembly, `src/analysis/semantic/DesignGraphProvider.*` for module hierarchy, schematic, call hierarchy, and backward cone query assembly, `src/analysis/semantic/CodeActionProvider.*`, `src/analysis/semantic/DiagnosticProvider.*`, and `src/analysis/semantic/QueryCache.*`
...
- Workspace symbol query assembly belongs in `AstIndex`; `SemanticEngine` may prepare value-type AST index context and cache results, but should not grow new workspace symbol filtering or kind-mapping branches.
