...
- current validation scale is 337+ focused unit `TEST_CASE`s and 292+ JSON semantic golden fixtures; the next maturity target is 340+ unit `TEST_CASE`s and 320+ golden fixtures focused on true complex-SV provider behavior plus fixture-driven differential, cache, affected-rebuild, stability-boundary, and perf coverage rather than debt-only fixtures
...
- Snapshot build input normalization, query-cache keying/stats, affected-dependency traversal, background diagnostics worker changes, and gate-runner/test-infrastructure changes must run the nearest focused tests, the full Debug long gate, Windows `clang-cl`, and a clean Release rebuild plus smoke tests before handoff.
...
- After completing and testing feature work in the Debug build, run a clean Release validation pass. Prefer `python scripts/run_clean_release_gate.py --build-dir build/release`; it deletes `build/release` only after a workspace absolute-path guard, reconfigures/rebuilds Release, runs `--version`, and runs the LSP, waveform, and layout release smoke tests. If running manually, perform the same guarded delete before `cmake --preset release`, `cmake --build --preset release`, and the relevant release smoke tests. If the Release build or smoke tests cannot be run in the current environment, record the reason in the final handoff.
...
The status runner summary is part of the gate contract: it records schema version, start/end time, duration, manifest hash, build preset/type, artifact root, the discovered CTest list, selected tests, ctest path, selected environment variables, required/optional skip counts, missing manifest tests, unclassified CTests, and required environment satisfaction. The 15 product/integration CTest targets are tracked in the runner manifest; all non-differential product gates are required, and `pristine_differential_slang_server` is the only optional skip. A missing manifest target, a non-optional skip, an unclassified CTest, missing required environment, or any failed test must be treated as a failed full gate.
...
Use the amd64 VS developer shell for local Windows clang-cl gates; an x86 `LIB`/toolchain environment can produce misleading CRT/STL link failures unrelated to the code. GitHub CI's Ubuntu 22.04 and 24.04 build-test matrix uses clang++ and is the required non-Windows compile gate for every PR. For layout/parser C++ changes, run an equivalent non-MSVC configure/build/test pass locally when WSL, Linux, a container, or native clang++ is available:
...
```powershell
python scripts/run_clean_release_gate.py --build-dir build/release
```
...
The clean Release summary should also record `gitHead`, `gitDirty`, `deletedPath`, and `releaseVersionOutput` so a handoff can prove the smoke tests used a freshly rebuilt binary.
...
`build/gate-artifacts/index.json` must fail generation if the full Debug summary is not clean, if required summaries are missing, or if the clean Release summary does not prove `build/release` was deleted before rebuild. Any added/removed summary contract fields must update runner script tests in the same change.
