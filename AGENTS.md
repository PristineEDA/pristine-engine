...
- current validation scale is 531+ focused unit `TEST_CASE`s and 495+ JSON semantic golden fixtures; the next maturity target is 550+ unit `TEST_CASE`s and 520+ golden fixtures focused on cross-module connection slice facts, parameter-override signed/width precision, slice-aware affected rebuild, shared-LSP differential, and required synthetic perf coverage rather than debt-only fixtures
