...
- current validation scale is 554+ focused unit `TEST_CASE`s and 521+ JSON semantic golden fixtures; the next maturity target is 570+ unit `TEST_CASE`s and 545+ golden fixtures focused on typed schematic connection projection, identity-based net grouping, module/interface/parameter affected rebuild, shared-LSP differential, and required synthetic perf coverage rather than debt-only fixtures
...
- resolved module, interface, and parameter schematic connections are projected during graph build into deterministic `SnapshotSchematicConnectionFact` values keyed by caller, instance, endpoint, source identity, and slice. `DesignGraphProvider` groups nets by source stable-id plus slice and uses `display_label` only as a presentation projection; it must not use `SchematicConnection.signal`, port name/index recovery, source text, `syntaxPortConnectionsForInstance`, `parameterOverrideConnectionsForAstInstance`, or query-time fallback as semantic truth. `SchematicCellFactIndexer` lowers primitive AST port connections into build-time `SnapshotSchematicCellPinFact` values, and `DesignGraphIndexBuilder` projects assignment sources and sinks into that same typed pin model. `DesignGraphProvider` may only consume `schematic_cell_pins_by_module`; primitive and assignment connection text is display-only and never a semantic source. Literal, dynamic, aggregate, and unresolved cell pins remain `partial/messages` rather than synthetic nets. Schematic telemetry must show local typed-fact/source-part and cell-pin work with `graphScannedGlobalSymbols == 0`; document, include/export, instance, parameter, watched-file, and config changes require visible hierarchy/schematic/cone/call-hierarchy invalidation coverage.
