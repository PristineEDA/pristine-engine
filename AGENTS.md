...
- Snapshot build input normalization, query-cache keying/stats, affected-dependency traversal, and background diagnostics worker changes must run the nearest focused unit tests, the full Debug long gate, Windows `clang-cl`, and a clean Release rebuild plus smoke tests before handoff.
...
- After completing and testing feature work in the Debug build, run a clean Release validation pass: first delete `build/release` after verifying the resolved absolute path is inside the repository workspace, then run `cmake --preset release`, `cmake --build --preset release`, and the relevant release smoke tests for touched subsystems. If the Release build or smoke tests cannot be run in the current environment, record the reason in the final handoff.
