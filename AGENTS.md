...
- current validation scale is 592+ focused unit `TEST_CASE`s and 533+ JSON semantic golden fixtures; the next maturity target is 610+ unit `TEST_CASE`s and 560+ golden fixtures focused on assertion invocation expansion, typed schematic cell-pin projection, identity-based net grouping, module/interface/parameter affected rebuild, shared-LSP differential, and required synthetic perf coverage rather than debt-only fixtures
...
- assertion and clocking semantics are build-time value facts: concurrent and immediate assertions create `SnapshotAssertionObservationFact` roots with typed `sampled`, `clock`, `disable`, and `abort` sources; explicit property clocking lowers through the timing-control collector, while scoped `default clocking` and `default disable iff` lower into `SnapshotAssertionContextFact` sources with `AssertionDefaultClock` / `AssertionDefaultDisable` origins. Explicit clocking or disable always wins over defaults. Property, sequence, and let invocations are `CallableInvocationFact` entries with source-ordered argument ranges mapped to declaration-order formal indexes and paired `SnapshotAssertionInvocationBindingFact` values, so signatureHelp and inlayHint never infer named actual bindings from text. `SemanticIndexVisitor` tags the resolved slang assertion-instance body sources with a stable invocation identity and expands any remaining formal source through its exact AST argument binding before `AssertionSample` cone edges are built. Missing, recursive, timing, or unprovable bindings become explicit partial facts; providers must never recover a formal, actual, or assertion body through AST pointers, text, names, or workspace scans. `AssertionSample` cone edges, unresolved facts, and root selections are built before `DesignGraphProvider` runs, so `backwardCone`, navigation, cache invalidation, and required synthetic perf use generation-bound indexes. Completion classifies Property/Sequence/Let/Checker/ClockingBlock candidates without query-time AST, text, symbol-table, stable-id substring, or workspace fallback. Do not add a second assertion parser or recover assertion clocks, disables, invocations, or actuals in `ServerSession`, `SemanticWorkspace`, providers, or `CompilationService`.
