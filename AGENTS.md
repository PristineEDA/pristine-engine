...
- HDL module hierarchy, schematic, and call hierarchy assembly are owned by `DesignGraphProvider` behind `SemanticEngine` value-type APIs; backward cone remains on the design-graph migration path and must not add syntax/text fallback branches
...
- split-out internal analysis providers for the large engine implementation: `src/analysis/semantic/SnapshotBuilder.*`, `src/analysis/semantic/AstIndex.*`, `src/analysis/semantic/NavigationProvider.*`, `src/analysis/semantic/CompletionProvider.*` for completion context detection, item construction, resolve docs/snippets/data, filtering, and completion-specific query helpers, `src/analysis/semantic/SignatureInlayProvider.*` for signature help and inlay hint query assembly, `src/analysis/semantic/DesignGraphProvider.*` for module hierarchy, schematic, and call hierarchy query assembly, `src/analysis/semantic/CodeActionProvider.*`, `src/analysis/semantic/DiagnosticProvider.*`, and `src/analysis/semantic/QueryCache.*`
...
- Module hierarchy, schematic, and call hierarchy assembly belongs in `DesignGraphProvider`; `SemanticEngine` may prepare value-type design graph context and cache results, but should not grow new HDL graph semantic branches.
