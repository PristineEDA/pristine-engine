...
- current validation scale is 508+ focused unit `TEST_CASE`s and 475+ JSON semantic golden fixtures; the next maturity target is 530+ unit `TEST_CASE`s and 495+ golden fixtures focused on cross-module connection slice facts, parameter-override precision, slice-aware affected rebuild, shared-LSP differential, and required synthetic perf coverage rather than debt-only fixtures
- cross-module connection slices are build-time `SnapshotResolvedConnectionSliceFact` values. Resolved port expressions and `ParameterValueAssignmentSyntax` plus URI-local AST reference facts may be used only while `AstIndex` is building; the temporary parameter syntax registry must be cleared before providers run. `DesignGraphIndexBuilder` resolves a child endpoint once through the typed endpoint map, then emits stable-id adjacency only. Do not recover semantic sources from `SchematicConnection.signal`, source text, whole symbol/module/assignment scans, or query-time fallbacks. Dynamic and unprovable interface/modport slices remain `dynamic`, `aggregate`, or `partial/messages`, never exact. Optional connection-slice telemetry is allowed without changing required wire fields.
