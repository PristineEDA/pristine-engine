...
- current validation scale is 326+ focused unit `TEST_CASE`s and 292+ JSON semantic golden fixtures; the next maturity target is 340+ unit `TEST_CASE`s and 320+ golden fixtures focused on true complex-SV provider behavior plus fixture-driven differential, cache, affected-rebuild, and perf coverage rather than debt-only fixtures
