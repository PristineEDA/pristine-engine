...
- current validation scale is 597+ focused unit `TEST_CASE`s and 536+ JSON semantic golden fixtures; the next maturity target is 610+ unit `TEST_CASE`s and 560+ golden fixtures focused on assertion temporal provenance, typed schematic cell-pin projection, identity-based net grouping, module/interface/parameter affected rebuild, shared-LSP differential, and required synthetic perf coverage rather than debt-only fixtures
- cross-module connection slices are build-time `SnapshotResolvedConnectionSliceFact` values. `ConnectionSliceIndexer` is the only lowering coordinator for resolved port and parameter overrides: it joins an exact instance syntax fact to the live AST instance, derives the child parameter endpoint from the definition declaration's URI-local range index, and lowers expression sources from resolved AST or, when slang marks that same parameter expression invalid, from its exact syntax-node range plus URI-local AST reference facts while the snapshot is building. Static select syntax produces exact slices; dynamic selectors produce explicit data/control partial facts. It clears syntax and AST-pointer scratch state before providers run. `DesignGraphIndexBuilder` resolves a child endpoint once through the typed endpoint id; it must not recover by module/name/index. Do not recover semantic sources from `SchematicConnection.signal`, source text, whole symbol/module/assignment scans, or query-time fallbacks. Dynamic and unprovable interface/modport slices remain `dynamic`, `aggregate`, or `partial/messages`, never exact. Optional connection-slice telemetry, including exact parameter-override mappings, is allowed without changing required wire fields.
- resolved module, interface, and parameter schematic connections are projected during graph build into deterministic `SnapshotSchematicConnectionFact` values keyed by caller, instance, endpoint, source identity, and slice. `DesignGraphProvider` groups nets by source stable-id plus slice and uses `display_label` only as a presentation projection; it must not use `SchematicConnection.signal`, port name/index recovery, source text, `syntaxPortConnectionsForInstance`, `parameterOverrideConnectionsForAstInstance`, or query-time fallback as semantic truth. `SchematicCellFactIndexer` lowers primitive, assignment, and operator AST-derived facts into build-time `SnapshotSchematicCellPinFact` values, including typed data/control roles, static slices, and deterministic generated operator-output identities. `DesignGraphIndexBuilder` only projects those values into per-module sorted/deduped pin views. `DesignGraphProvider` may only consume `schematic_cell_pins_by_module`; primitive, assignment, and operator connection text is display-only and never a semantic source. Literal, dynamic, aggregate, and unresolved cell pins remain `partial/messages` rather than synthetic nets. Schematic telemetry must show local typed-fact/source-part and cell-pin work with `graphScannedGlobalSymbols == 0`; document, include/export, instance, parameter, watched-file, and config changes require visible hierarchy/schematic/cone/call-hierarchy invalidation coverage.
- resolved interface/modport module connections are build-time `SnapshotInterfaceMemberConnectionFact` values. `DesignGraphIndexBuilder` must join child and parent modport members through their `internal_symbol_stable_id`, retain child endpoint, interface-instance, member, direction, range, and generated-instance identities, and emit `InterfaceMember` cone edges from those facts only. `DesignGraphProvider` projects interface nets and cell connections from `interface_member_connections_by_module`; a bare interface connection without resolved modport members remains structural only and must not create a generic `interface` net, while an unresolved modport fact returns `partial/messages`. Do not match member names, parse interface connection text, scan symbols, or restore a generic interface-net fallback.
- primitive cell semantics are build-time value facts: `SchematicCellFactIndexer` pre-indexes primitive cell URI/range bindings plus stable display labels while the AST is alive, classifies `bufif`/`notif`/MOS control pins, `cmos`/`rcmos` dual controls, and `tranif`/`rtranif` enables as typed `Control` pins, and `DesignGraphIndexBuilder` emits `PrimitiveCell` data edges plus `PrimitiveControl` control edges into `ConeAdjacencyIndex`. `DesignGraphProvider` may only consume these prebuilt facts; it must not use primitive connection strings, source text, port names, instance/module scans, or query-time AST recovery. Static primitive pins may form exact/whole adjacency; literal, dynamic, aggregate, and unresolved pins remain explicit `partial/messages`. Primitive schematic/cone telemetry is optional, but `graphScannedGlobalSymbols` and `coneScannedGlobalEdges` remain zero.
- typed schematic response projection is mandatory: `SemanticSchematicCell` and `SemanticSchematicConnection` are provider-owned response values generated exclusively from `schematic_connections_by_module`, `schematic_cell_ids_by_instance_id`, and `schematic_cell_pins_by_module`. Raw `SchematicCell::connections` and `SchematicConnection.signal` are structural compatibility fields only and cannot create a graph net, response connection, operator pin, or missing-port quickfix. `CodeActionProvider` resolves missing module ports through `connection_bindings_by_instance_id` plus typed endpoint facts; if the exact instance binding is absent, it must return no speculative quickfix. Do not restore module/name/range matching, port-index/name recovery, source text, or `CompilationService` fallback.
- module signature help and module-port/parameter inlay hints must consume `SignatureInlayConnection` values projected from `resolved_connection_slices_by_instance_id` and typed endpoint facts while `AstIndex` is building. The provider must not read `SchematicCell::connections`, `SnapshotModuleInstance::port_connections`, or `parameter_connections`; missing typed bindings yield no speculative labels or active-parameter ranges.
...
- assertion and clocking semantics are build-time value facts: concurrent and immediate assertions create `SnapshotAssertionObservationFact` roots with typed `sampled`, `clock`, `disable`, `abort`, and conditional/case `control` sources; explicit property clocking lowers through the timing-control collector, while scoped `default clocking` and `default disable iff` lower into `SnapshotAssertionContextFact` sources with `AssertionDefaultClock` / `AssertionDefaultDisable` origins. Explicit clocking or disable always wins over defaults. `SnapshotAssertionTemporalFact` records source-ordered delay, repetition, unary/binary temporal operator, conditional-branch, and case-branch provenance on each assertion source; selectors and case matches must remain `control`, never `sampled`. Property, sequence, and let invocations are `CallableInvocationFact` entries with source-ordered argument ranges mapped to declaration-order formal indexes and paired `SnapshotAssertionInvocationBindingFact` values, so signatureHelp and inlayHint never infer named actual bindings from text. `SemanticIndexVisitor` tags the resolved slang assertion-instance body sources with a stable invocation identity and expands any remaining formal source through its exact AST argument binding before `AssertionSample` cone edges are built. Missing, recursive, timing, or unprovable bindings become explicit partial facts; providers must never recover a formal, actual, temporal path, or assertion body through AST pointers, text, names, or workspace scans. `AssertionSample` cone edges, unresolved facts, temporal paths, and root selections are built before `DesignGraphProvider` runs, so `backwardCone`, navigation, cache invalidation, and required synthetic perf use generation-bound indexes. `DesignGraphProvider` may only project temporal facts by typed adjacency key; it must not rediscover them from expression text. Completion classifies Property/Sequence/Let/Checker/ClockingBlock candidates without query-time AST, text, symbol-table, stable-id substring, or workspace fallback. Do not add a second assertion parser or recover assertion clocks, disables, invocations, temporal relations, or actuals in `ServerSession`, `SemanticWorkspace`, providers, or `CompilationService`.
...
- sequential event-control cone facts are also build-time only: `SemanticIndexVisitor` lowers each `TimedStatement` into `SnapshotConeEventControlFact` plus `EventControl` / `EventIff` seeds while slang timing controls are alive. Signal events retain `any` / `posedge` / `negedge` / `bothEdges`, event lists retain each source occurrence, and `iff` guards are distinct control edges. `@*` records an implicit timing fact but must not synthesize a clock edge; delays or unsupported timing forms remain explicit unresolved partial facts. Assertion default clocking and disable are covered by the same provider-owned adjacency path and must retain explicit/default precedence, zero global scans, and warm-cache coverage. Providers may only consume typed adjacency and event metadata, never timing AST pointers, timing text, name guesses, whole-workspace scans, or `CompilationService` fallback. Synthetic perf must exercise cold/warm sequential and assertion-default cone queries with local adjacency work and zero global scans; every semantic round still runs focused tests, full Debug/perf, native VS amd64 `clang-cl` Debug/Release, guarded clean canonical Release, and a same-run artifact index.
...
- code actions for unresolved include/module/type and missing port connections are produced through `CodeActionProvider` behind `SemanticEngine` value-type APIs. Missing-port quickfixes require the exact typed instance-to-endpoint binding; `ServerSession` only serializes them to LSP JSON.
