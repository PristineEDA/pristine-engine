...
- current validation scale is 439+ focused unit `TEST_CASE`s and 405+ JSON semantic golden fixtures; the next maturity target is 455+ unit `TEST_CASE`s and 430+ golden fixtures focused on URI-local navigation targets, implementation edges, visible-query affected rebuild, fixture-driven differential, and required synthetic perf coverage rather than debt-only fixtures
