...
- `WorkspaceDiscoveryIndex` provides a lightweight initialize/warm-path discovery index for module/interface/package/macro/file candidates, top-candidate hints, include plus package import/export dependency-closure hints, closure metrics, and timing counters; `SemanticEngine` caches only the value-type discovery snapshot with a generation/config/file-set cache key, so it may guide closure-limited hierarchy/schematic deep builds but must not become the final semantic authority for hover, definition, hierarchy, schematic, cone, diagnostics, or code actions. Hierarchy/schematic may expose optional `discoveryClosure*` telemetry such as root, candidate/selected/missing/deduped document counts, and build timing, but those fields are perf/debug data only
- current validation scale is 220+ focused unit `TEST_CASE`s and 270+ JSON semantic golden fixtures; the next maturity target is true complex-SV provider behavior plus fixture-driven differential, cache, affected-rebuild, and perf coverage rather than debt-only fixtures
...
- Discovery closure metrics are perf/debug value-type summaries only. They may report root, candidate/selected/missing/deduped documents, cache behavior, and closure build timing, but they must not be serialized as mandatory LSP semantics or used to override `AstIndex` / design graph answers.
