...
- `tests/gate_manifest.json` is the single source of truth for required and optional full-gate CTest targets. Any added, renamed, or removed product/integration CTest gate must update this manifest, runner script tests, and any CI/docs references in the same change. `pristine_script_runner_tests` remains a runner self-test and must not be counted as a product gate in the manifest.
...
The status runner summary is part of the gate contract: it records the discovered CTest list, selected tests, build dir, ctest path, selected environment variables, required/optional skip counts, and missing manifest tests. The 15 product/integration CTest targets are tracked in the runner manifest; all non-differential product gates are required, and `pristine_differential_slang_server` is the only optional skip. A missing manifest target, a non-optional skip, or any failed test must be treated as a failed full gate.
