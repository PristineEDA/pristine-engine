...
- current validation scale is 584+ focused unit `TEST_CASE`s and 530+ JSON semantic golden fixtures; the next maturity target is 600+ unit `TEST_CASE`s and 560+ golden fixtures focused on assertion/clocking facts, typed schematic cell-pin projection, identity-based net grouping, module/interface/parameter affected rebuild, shared-LSP differential, and required synthetic perf coverage rather than debt-only fixtures
...
- assertion and clocking semantics are also build-time value facts: concurrent and immediate assertions create `SnapshotAssertionObservationFact` roots with typed `sampled`, `clock`, `disable`, and `abort` sources; explicit property clocking lowers through the existing timing-control collector, while malformed source expressions remain `partial/messages`. `AssertionSample` cone edges, unresolved facts, and root selections are built before `DesignGraphProvider` runs, so `backwardCone`, navigation, and cache invalidation use the normal generation-bound indexes. Property, sequence, and let invocations are `CallableInvocationFact` entries for signature help, and completion classifies Property/Sequence/Let/Checker/ClockingBlock candidates without query-time AST, text, symbol-table, stable-id substring, or workspace fallback. Do not add a second assertion parser or recover assertion sources in `ServerSession`, `SemanticWorkspace`, providers, or `CompilationService`.
...
- sequential event-control cone facts are also build-time only: `SemanticIndexVisitor` lowers each `TimedStatement` into `SnapshotConeEventControlFact` plus `EventControl` / `EventIff` seeds while slang timing controls are alive. Signal events retain `any` / `posedge` / `negedge` / `bothEdges`, event lists retain each source occurrence, and `iff` guards are distinct control edges. `@*` records an implicit timing fact but must not synthesize a clock edge; delays or unsupported timing forms remain explicit unresolved partial facts. Providers may only consume typed adjacency and event metadata, never timing AST pointers, timing text, name guesses, whole-workspace scans, or `CompilationService` fallback. Assertion sampled-event semantics remain a separate future task. Synthetic perf must exercise cold/warm sequential cone queries with an event-control edge, timing-fact lookup, local adjacency work, and zero global scans; every semantic round still runs focused tests, full Debug/perf, native VS amd64 `clang-cl` Debug/Release, guarded clean canonical Release, and a same-run artifact index.
