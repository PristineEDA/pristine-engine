...
- current validation scale is 363+ focused unit `TEST_CASE`s and 320+ JSON semantic golden fixtures; the next maturity target is 380+ unit `TEST_CASE`s and 350+ golden fixtures focused on AST-backed scope/callable visibility, package import/export semantics, provider-owned completion, visible-query affected rebuild, fixture-driven differential, and required synthetic perf coverage rather than debt-only fixtures
...
- current semantic feature focus is AST-backed scope and callable visibility: `AstIndex` owns lexical/enclosing scope candidates with source-backed parent/context metadata, explicit and wildcard imports, transitive package exports with cycle guards, typed receiver members, URI-local inlay symbol facts, include-closure macro visibility, and AST-backed callable facts. `CompletionProvider` owns completion context, filtering, deterministic precedence, dedupe, snippets, resolve metadata, and lower-bound workspace prefix selection. Completion queries must not scan the full symbol table, use stable-id substring or fuzzy membership tests, scan all workspace macros, or fall back to syntax/text-derived semantic answers. An unresolved package, receiver, or import returns no speculative global candidates and an explicit message; local/member/package paths do not consult the workspace prefix index
