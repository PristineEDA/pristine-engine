...
- current validation scale is 340+ focused unit `TEST_CASE`s and 292+ JSON semantic golden fixtures; the next maturity target is 345+ unit `TEST_CASE`s and 320+ golden fixtures focused on true complex-SV provider behavior plus fixture-driven differential, cache, affected-rebuild, stability-boundary, Release-only compile, and perf coverage rather than debt-only fixtures
