...
- macro-function and AST-backed function/task signature help are owned by `SignatureInlayProvider` and must consume `AstIndex` / snapshot macro and call views; do not reimplement argument parsing or active-parameter calculation in `ServerSession`
...
- mature completion, signature, and inlay providers with package `::`, hierarchical `.`, ordered/named/wildcard ports, resolved type, constant value, and interface/modport cases
