...
- current validation scale is 415+ focused unit `TEST_CASE`s and 380+ JSON semantic golden fixtures; the next maturity target is 430+ unit `TEST_CASE`s and 400+ golden fixtures focused on URI-local reference roles, generated/repeated module call edges, visible-query affected rebuild, fixture-driven differential, and required synthetic perf coverage rather than debt-only fixtures
...
- reference and module-call queries use build-time value facts: `ReferenceOccurrenceIndex` owns URI-local declaration/read/write/type/instance occurrences plus deterministic same-range alias views, while `ModuleCallEdgeIndex` owns caller/callee/instance identities and generated or repeated edges. Hover, definition, references, rename, prepareRename, and documentHighlight must locate a symbol through the URI-local occurrence range index; call hierarchy prepare/incoming/outgoing must use generation-bound opaque item ids and caller/callee edge maps. Query-time scans across all references or module entries, name/range reconstruction, stale opaque-id fallback, and text-based symbol guessing are prohibited. Explicit `topModules` snapshots use full slang elaboration rather than lint-only uninstantiated definitions, and loop-generate arrays must visit each elaborated entry so repeated generated instances keep independent identities
