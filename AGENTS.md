...
- current validation scale is 580+ focused unit `TEST_CASE`s and 528+ JSON semantic golden fixtures; the next maturity target is 600+ unit `TEST_CASE`s and 550+ golden fixtures focused on sequential event-control cone facts, typed schematic cell-pin projection, identity-based net grouping, module/interface/parameter affected rebuild, shared-LSP differential, and required synthetic perf coverage rather than debt-only fixtures
