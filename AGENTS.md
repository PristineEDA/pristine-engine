...
- resolved module, interface, and parameter schematic connections are projected during graph build into deterministic `SnapshotSchematicConnectionFact` values keyed by caller, instance, endpoint, source identity, and slice. `DesignGraphProvider` groups nets by source stable-id plus slice and uses `display_label` only as a presentation projection; it must not use `SchematicConnection.signal`, port name/index recovery, source text, `syntaxPortConnectionsForInstance`, `parameterOverrideConnectionsForAstInstance`, or query-time fallback as semantic truth. Dynamic, aggregate, literal, and unresolved connections remain `partial/messages` rather than synthetic nets. Primitive and assignment cell text is display-only and is not a semantic authority. Schematic telemetry must show local typed-fact/source-part work with `graphScannedGlobalSymbols == 0`; document, include/export, instance, parameter, watched-file, and config changes require visible hierarchy/schematic/cone/call-hierarchy invalidation coverage.
