...
- current validation scale is 446+ focused unit `TEST_CASE`s and 405+ JSON semantic golden fixtures; the next maturity target is 470+ unit `TEST_CASE`s and 440+ golden fixtures focused on `GraphEndpointFact`, `ConeAdjacencyIndex`, visible graph-query affected rebuild, shared-LSP differential, and required synthetic perf coverage rather than debt-only fixtures
...
- design graph queries use build-time value facts: `DesignGraphBindingIndex` owns exact URI/range-to-symbol, module-scope/name, port, parameter, generated-instance, and `GraphEndpointFact` bindings; endpoint facts retain stable identity, kind, direction, source range, and generated-instance identity. `ConeAdjacencyIndex` owns stable assignment and instance-port edges, including edge kind and generated-instance identity, in both directions. `DesignGraphProvider` may directly look up node metadata by stable ID, but must not scan `symbols_by_id`, module entries, per-URI assignment maps, or reconstruct graph identity by name/range at query time. Missing module endpoint bindings return `partial/messages`; they do not re-enable syntax, text, workspace, or port-name direction guesses such as `y/out/o/q`. `graphScannedGlobalSymbols` and `coneScannedGlobalEdges` must stay zero in focused, synthetic perf, and RTL E2E summaries; synthetic cross-module cone probes must also demonstrate a nonzero local adjacency scan.
