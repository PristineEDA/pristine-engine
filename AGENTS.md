...
- current validation scale is 190+ focused unit `TEST_CASE`s and 230+ JSON semantic golden fixtures; the next maturity target is true complex-SV provider behavior plus fixture-driven differential, cache, affected-rebuild, and perf coverage rather than debt-only fixtures
...
- JSON semantic golden fixtures for observable provider behavior, currently 230+ fixtures covering hover, definition/typeDefinition, references, rename, completion/resolve, signatureHelp, inlayHint, semanticTokens, selectionRange, diagnostics, codeAction, workspace/symbol, moduleHierarchy, schematic, backwardCone, and callHierarchy: `tests/golden/semantic`
