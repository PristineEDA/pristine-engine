...
- LEF/DEF/GDS parsing, recovery diagnostics, layout data normalization, GDS cell/reference/element hierarchy modeling, hierarchy flattening, catalog encoding, geometry filtering/truncation, and columnar frame encoding belong in `src/layout/*`; do not add LEF/DEF/GDS grammar or physical-layout encoding rules to `ServerSession.cpp`. `ServerSession` may validate `lefUris` / `defUri` / `gdsUri`, enforce workspace-root path boundaries, start/close sessions, choose the source, and serialize session metadata only.
...
- Frontend-facing layout payloads should stay columnar binary and typed-array-friendly. Prefer fixed headers plus offset/count fields for `Uint32Array`, `Float64Array`, and `Uint8Array` views; keep layer/macro/net/cell labels in a lazy string table, and keep large geometry responses filtered and truncatable instead of serializing per-shape JSON over LSP. `pristine-layout-columnar-v2` keeps the `PLD1` envelope and uses catalog/geometry payload version 2; `PLCT` keeps its 80-byte header and uses header offset 72 as `u32 pinCount` plus offset 76 as `u32 pinTableOffset`. `PLCT` pin table rows are 28 bytes: `u32 macroIndex`, `u32 pinIndex`, `u32 nameStringOffset`, `u32 useStringOffset`, `u16 direction`, `u16 reserved`, `u32 firstShapeIndex`, `u32 shapeCount`; this table covers LEF macro pins only, including names such as `VDD` and `VSS`, while DEF top-level pins remain outside it. `pristine-layout-columnar-v3` is for GDS sessions: it keeps the same `PLD1` frame envelope, uses `PLCT` payload version 3 with a 128-byte header, adds GDS cell/reference/element/point tables, and reuses `PLGE` geometry shape rows for flattened render geometry. `PLGE` shape table entries are 28 bytes: `u32 layerIndex`, `u16 kind`, `u16 ownerKind`, `u32 ownerIndex`, `u32 macroIndex`, `u32 flags`, `u32 polygonOffset`, `u32 polygonPointLength`. `macroIndex == 0xffffffff` means no macro owner. LEF macro pin and obstruction shapes must set `macroIndex` to the stable `catalog.macros` index; GDS flattened shapes set `ownerKind == GdsElement`, `ownerIndex` to the GDS element index, and `macroIndex` to the source GDS cell index. Frontends must map pin shapes to names with `(shape.macroIndex, shape.ownerIndex) -> (pinTable.macroIndex, pinTable.pinIndex)` and must filter macro-local geometry by `shape.macroIndex === selectedMacro.index`, not by bounds intersection. Legacy v1 layout frames are unsupported and should be rejected or followed by reopening the session.
...
When reporting release validation for layout protocol/header changes, include the actual `systemverilog/layout/open` metadata observed from the release binary: LEF/DEF should report `protocol == "pristine-layout-columnar-v2"` and `source == "lefdef"`, while GDS should report `protocol == "pristine-layout-columnar-v3"` and `source == "gds"`.
