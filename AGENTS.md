...
- completion, completion resolve, signature help, diagnostics, inlay hints, semantic tokens, selection ranges, and workspace symbols use `SemanticEngine` value-type APIs; completion context detection is owned by `CompletionProvider`, and remaining context gaps belong in analysis instead of `ServerSession`
...
- split-out internal analysis providers for the large engine implementation: `src/analysis/semantic/SnapshotBuilder.*`, `src/analysis/semantic/AstIndex.*`, `src/analysis/semantic/NavigationProvider.*`, `src/analysis/semantic/CompletionProvider.*` for completion context detection and completion-specific helpers, `src/analysis/semantic/SignatureInlayProvider.*`, `src/analysis/semantic/DesignGraphProvider.*`, `src/analysis/semantic/CodeActionProvider.*`, `src/analysis/semantic/DiagnosticProvider.*`, and `src/analysis/semantic/QueryCache.*`
...
- Completion context detection for package `::`, hierarchical `.`, macro, and module-instantiation positions belongs in `CompletionProvider`; do not duplicate that logic in `SemanticEngine` or `ServerSession`.
