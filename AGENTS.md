...
- split-out internal analysis providers for the large engine implementation: `src/analysis/semantic/SnapshotBuilder.*` for SourceManager/SyntaxTree/Compilation construction, snapshot data ownership, slang diagnostics, snapshot utilities, and include/import/module dependency edges; `src/analysis/semantic/AstIndex.*` for AST symbol identity, declaration/reference index construction, module instance binding, workspace symbol query assembly, and provider-facing symbol/reference/module/signature/diagnostic/design-graph views; `src/analysis/semantic/NavigationProvider.*` for semantic token and selection range query assembly, `src/analysis/semantic/CompletionProvider.*` for completion context detection, item construction, resolve docs/snippets/data, filtering, and completion-specific query helpers, `src/analysis/semantic/SignatureInlayProvider.*` for signature help and inlay hint query assembly, `src/analysis/semantic/DesignGraphProvider.*` for module hierarchy, schematic, call hierarchy, and backward cone query assembly, `src/analysis/semantic/CodeActionProvider.*` for quickfix selection and edit/create-file assembly, `src/analysis/semantic/DiagnosticProvider.*` for slang + UX diagnostic aggregation/sort/dedupe, and `src/analysis/semantic/QueryCache.*`
